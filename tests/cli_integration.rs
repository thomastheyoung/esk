...
fn deploy_records_tombstone_delete_success() {
    ...
    // strict: false because STRIPE_KEY is now deleted (required but missing)
    let runner = MockCommandRunner::new();
    runner.push_success(b"", b""); // preflight: wrangler --version
    runner.push_success(b"", b""); // preflight: wrangler whoami
...
fn deploy_records_tombstone_delete_failure() {
    ...
    // strict: false because STRIPE_KEY is deleted (required but missing)
    let runner = MockCommandRunner::new();
    runner.push_success(b"", b""); // preflight: wrangler --version
    runner.push_success(b"", b""); // preflight: wrangler whoami
...
fn deploy_retries_failed_tombstone_delete() {
    ...
    // First deploy: delete fails (strict: false — STRIPE_KEY deleted)
    let runner = MockCommandRunner::new();
    runner.push_success(b"", b""); // preflight: wrangler --version
    runner.push_success(b"", b""); // preflight: wrangler whoami
...
fn delete_then_recreate_same_value_syncs() {
    ...
    // Delete and sync (tombstone processed, strict: false — deleted key is required)
    store.delete("STRIPE_KEY", "dev").unwrap();
    let runner = MockCommandRunner::new();
    runner.push_success(b"", b""); // preflight: wrangler --version
    runner.push_success(b"", b""); // preflight: wrangler whoami
...
fn generate_env_example_default() {
    ...
    // Env examples use a comment header, which must authorize a subsequent
    // generated overwrite just like TypeScript's slash-comment header does.
    cli::generate::run(&config, Some(&GenerateFormat::EnvExample), None, false).unwrap();
}
...
#[cfg(unix)]
#[test]
fn generate_preflight_rejects_post_load_symlink_before_any_output_is_written() {
    use std::os::unix::fs::symlink;

    let project = TestProject::with_store(GENERATE_CONFIG).unwrap();
    let config = project.config().unwrap();
    let outside = project.root().join("outside-sentinel");
    std::fs::write(&outside, "# Generated by esk — outside sentinel\n").unwrap();
    symlink(&outside, project.root().join(".env.example")).unwrap();

    let err = cli::generate::run(&config, None, None, false).unwrap_err();
    assert!(err.to_string().contains("traverses a symlink"));
    assert!(!project.root().join("env.d.ts").exists());
    assert_eq!(
        std::fs::read_to_string(&outside).unwrap(),
        "# Generated by esk — outside sentinel\n"
    );
}
...
fn deploy_fails_with_missing_required() {
    ...
    let config = project.config().unwrap();
    std::fs::create_dir_all(project.root().join("apps/web")).unwrap();
    // DB_URL is required but not set → should fail with --strict
    let store = project.store().unwrap();
...
fn deploy_force_overrides_required() {
    ...
    let config = project.config().unwrap();
    std::fs::create_dir_all(project.root().join("apps/web")).unwrap();
    // DB_URL missing but force=true overrides strict
    cli::deploy::run(
        &config,
        &cli::deploy::DeployOptions {
            env: Some("dev"),
...
fn deploy_required_env_scoped_fails_in_prod() {
    ...
    // SENTRY_DSN required in prod but not set — strict should fail
    let err = cli::deploy::run(
        &config,
        &cli::deploy::DeployOptions {
            env: Some("prod"),
...
fn deploy_dry_run_warns_empty_but_succeeds() {
    ...
    // dry_run warns but doesn't fail
    cli::deploy::run(
        &config,
        &cli::deploy::DeployOptions {
            env: Some("dev"),
...
fn deploy_dry_run_warns_cross_field() {
    ...
    let store = project.store().unwrap();
    store.set("AUTH_ENABLED", "dev", "true").unwrap();
    // AUTH_SECRET not set → dry_run should warn but not fail
