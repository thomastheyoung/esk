...
impl DeployTarget for DotenvTarget<'_> {
    ...
    fn deploy_batch(
        ...
        self.write_dotenv_file(app, &target.environment, secrets)?;
        Ok(secrets
            .iter()
            .map(|s| DeployResult {
                key: s.key.clone(),
...
"#;
    ...
    fn deploy_batch_no_app_errors() {
        ...
        let error = target
            .deploy_batch(&secrets, &make_target(None, "dev"))
            .unwrap_err();
    ...
    fn deploy_batch_writes_env_file() {
        ...
        let results = target
            .deploy_batch(&secrets, &make_target(Some("web"), "dev"))
            .unwrap();
...
"#,
        ...
        assert!(target
            .deploy_batch(&[], &make_target(Some("web"), "dev"))
            .is_err());
    }
    ...
    fn dotenv_read_back_returns_the_files_values() {
        ...
        target.deploy_batch(&secrets, &resolved).unwrap();
    ...
    fn dotenv_read_back_surfaces_an_edited_value_as_drift() {
        ...
        target
            .deploy_batch(&[make_secret("API_KEY", "correct", "G")], &resolved)
            .unwrap();
    ...
    fn dotenv_read_back_reports_a_deleted_file_as_missing_keys() {
        ...
        target
            .deploy_batch(&[make_secret("API_KEY", "v", "G")], &resolved)
            .unwrap();
        let path = config.dotenv_display_path("web", "dev").unwrap();
        std::fs::remove_file(&path).unwrap();
