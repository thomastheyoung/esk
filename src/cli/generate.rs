...
pub fn run(
    ...
    preflight_outputs(config, &outputs)?;
...
const GENERATED_HEADERS: [&str; 2] = ["// Generated by esk", "# Generated by esk"];

fn existing_output_is_esk_generated(path: &Path) -> Result<Option<bool>> {
    match std::fs::symlink_metadata(path) {
        Ok(metadata) => {
            ...
            Ok(Some(
                GENERATED_HEADERS
                    .iter()
                    .any(|header| contents.starts_with(header)),
            ))
        }
...
#[cfg(test)]
fn is_esk_generated(path: &Path) -> Result<bool> {
    Ok(existing_output_is_esk_generated(path)?.unwrap_or(false))
}
...
fn generate_one(
    ...
    // Revalidate immediately before header inspection: the filesystem may have
    // changed since preflight or config loading.
    let mut out_path = ensure_output_overwritable(config, entry)?;
    ...
    // Revalidate immediately before the write. This catches an output or a
    // newly-created parent that changed into a symlink after inspection.
    out_path = ensure_output_overwritable(config, entry)?;
...
fn output_name(entry: &GenerateOutput) -> &str {
    entry
        .output
        .as_deref()
        .unwrap_or(entry.format.default_output())
...
fn preflight_outputs(config: &Config, outputs: &[GenerateOutput]) -> Result<()> {
    for entry in outputs {
        ensure_output_overwritable(config, entry)?;
    }
    Ok(())
}

fn ensure_output_overwritable(config: &Config, entry: &GenerateOutput) -> Result<PathBuf> {
    let output = output_name(entry);
    let out_path = config.resolve_project_output_path(output)?;
    if matches!(existing_output_is_esk_generated(&out_path)?, Some(false)) {
        let relative = out_path.strip_prefix(&config.root).unwrap_or(&out_path);
        bail!(
...
mod tests {
    ...
    fn is_esk_generated_true() {
        ...
        assert!(is_esk_generated(&path).unwrap());
    }

    #[test]
    fn is_esk_generated_recognizes_env_example_header() {
        let dir = tempfile::tempdir().unwrap();
    ...
    fn is_esk_generated_false_for_missing_file() {
        let dir = tempfile::tempdir().unwrap();
        assert!(!is_esk_generated(&dir.path().join("nope.ts")).unwrap());
    }
