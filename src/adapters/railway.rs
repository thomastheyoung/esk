...
"#;
    ...
    fn railway_preflight_success() {
        ...
        let runner = MockRunner::new(vec![
            CommandOutput { success: true, stdout: b"3.0.0".to_vec(), stderr: vec![] },
            CommandOutput { success: true, stdout: b"user@test".to_vec(), stderr: vec![] },
        ]);
        let adapter = RailwayAdapter { config: &config, adapter_config, runner: &runner };
    ...
    fn railway_preflight_missing_cli() {
        ...
        let adapter = RailwayAdapter { config: &config, adapter_config, runner: &FailRunner };
        let err = adapter.preflight().unwrap_err();
    ...
    fn railway_sync_with_env_flags() {
        ...
        let calls = runner.take_calls();
        assert_eq!(calls[0].1, vec!["variables", "--set", "KEY=val", "--environment", "production"]);
    }
    ...
    fn railway_delete_with_env_flags() {
        ...
        let calls = runner.take_calls();
        assert_eq!(calls[0].1, vec!["variables", "delete", "KEY", "--environment", "production"]);
    }
